...
function resetHighlight() {
  ...
  edges.forEach(e => {
    edges.update({ id: e.id, opacity: 1.0, width: 1.5, color: { color: '#475569', highlight: '#f1f5f9', hover: '#94a3b8' } });
  });
  document.getElementById('statusMsg').textContent =
    'Ready — click a node to highlight its lineage path';
}
...
function highlightNode(nodeId) {
  // Collect directly connected nodes and edges
  ...
  nodes.forEach(n => {
    nodes.update({ id: n.id, opacity: connectedNodes.has(n.id) ? 1.0 : DIM_OPACITY });
  });
  edges.forEach(e => {
    const active = connectedEdges.has(e.id);
    edges.update({
      id: e.id,
      opacity: active ? 1.0 : 0.05,
      width: active ? 2.5 : 1,
      color: active
  ...
  const upCount   = (edgeTo[nodeId]   || []).length;
  const downCount = (edgeFrom[nodeId] || []).length;
  document.getElementById('statusMsg').textContent =
    `Selected: ${nd.model}.${nd.column}  |  ${upCount} upstream  ·  ${downCount} downstream`;
}
